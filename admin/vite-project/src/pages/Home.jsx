...
const fetchCounts = async () => {
  try {
    ...
    // API may return an object with `products` or an array directly
    const productsData = productsRes.data?.products ?? productsRes.data;
    const productsCount = Array.isArray(productsData) ? productsData.length : 0;
    setTotalProducts(productsCount);

    // Fetch orders list (admin endpoint)
    const ordersRes = await axios.post(`${serverUrl}/api/order/list`, {}, { withCredentials: true });
    const ordersData = ordersRes.data ?? [];
    const ordersCount = Array.isArray(ordersData) ? ordersData.length : (ordersData.success && Array.isArray(ordersData.orders) ? ordersData.orders.length : 0);
  ...
  return (
    <div className='w-[100vw] h-[100vh] bg-gradient-to-l from-[#141414] to-[#0c2025] text-[white] relative'>
      ...
      <div className='w-[70vw] h-[100vh] absolute left-[25%] flex items-start justify-start flex-col gap-[40px] py-[100px] '>
        <h1 className='text-[35px] text-[#afe2f2] '>YourCart Admin Panel</h1>
